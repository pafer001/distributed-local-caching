...
public class DateUtilTool
{
	...
	{
		return date == null ? null : format(date, getDatePattern());
	}
     ...
     * @return
     */
    public static String format(Long timeId){
        if(timeId == null) {
			return new SimpleDateFormat(defaultDatePattern).format(new Date());
		}  else {
			return new SimpleDateFormat(defaultDatePattern).format(new Date(timeId));
		}
    }
	...
	{
		return Strings.isNullOrEmpty(strDate) ? null : parse(strDate,
				getDatePattern());
	}
	...
	{
		return Strings.isNullOrEmpty(strDate) ? null : new SimpleDateFormat(
